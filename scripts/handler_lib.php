...
function change($mysql_conn, $action_change, $table ,$flat, $cash, $date){
	if ($action_change=='add'){
		$mysql_conn->query("insert into {$table}(number_flat,date,cash) values ({$flat},'{$date}',{$cash})");
	} else if ($action_change=='delete'){
		$mysql_conn->query("delete from {$table} where number_flat={$flat} and date='{$date}' and cash={$cash}");
	}
}
...
function show_payments($mysql_conn, $type_display, $from, $to, $year_month, $sort){
	...
	if ($type_display=="1"){
		$sql = "select * from payments where {$from}>=number_flat and {$to}<=number_flat and date='{$year_month}'";
...
function show_charges($mysql_conn, $from, $to, $year_month, $sort){
	$result  = array("action"=>"charges","id"=>array(),"flat"=>array(), "date"=>array(), "cash"=>array());
	$sql = "select * from charges";
	if ($type_display=="1"){
		$sql = "select * from charges where {$from}>=number_flat and number_flat<={$to}";
	} else if ($type_display=="2"){
...
function show_saldo($mysql_conn, $type_display, $from, $to, $year_month, $sort){
	...
	if ($type_display=="1"){
		$sql = "select * from saldo where {$from}<=number_flat and {$to}>=number_flat and date='{$date
		}'";
	} else if ($type_display=="2"){
