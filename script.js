...
function addDelTable(){
	...
	/*switch($('#action')[0].value){
		case "charges":
		case "payments":
			$('<th scope="col">#</th><th scope="col">Дата</th><th scope="col">Квартира</th><th scope="col">Сумма</th>')
			.appendTo('#trTable');
			break;
		case "saldo":
			$('<th scope="col">#</th><th scope="col">Дата</th><th scope="col">Квартира</th><th scope="col">Начисления</th><th scope="col">Платежи</th>')
			.appendTo('#trTable');
			break;
		case "sheet1":
			$('');
	...
	function handlerPayment(result, result_form){
		var str = '<thead><tr id = "trTable"><th score="col">id</th><th score="col">Квартира</th><th score="col">Дата</th><th score="col">Сумма</th>',
		body = "";
	...
	function handlerCharge(result, result_form){
		var str = '<thead><tr id = "trTable"><th score="col">id</th><th score="col">Квартира</th><th score="col">Дата</th><th score="col">Сумма</th></tr>',
		body = "";
		for (var i = 0, l = result["id"].length;i < l;i++){
			body+=`<tr><th score="col">${result["id"][`${i}`]}</th><th score="col">${result["flat"][`${i}`]}</th><th score="col">${result["date"][`${i}`]}</th><th score="col">${result["cash"][`${i}`]}</th></tr>`;
		};
	...
	function handlerSaldo(result, result_form){
		var str = '<thead><tr id = "trTable"><th score="col">id</th><th score="col">Квартира</th><th score="col">Дата</th><th score="col">Платеж</th><th score = "col">Начисления</th>',
		body = "";
		for (var i = 0, l = result["id"].length;i < l;i++){
	...
	function changeInputYearMonth(str)
	{
		if (str == "year"){
			...
			$('#inputYearMonth').attr("value", "2021");
			...
			$('#inputYearMonth').attr("value", "2021-12");
	...
	$(document).ready(function(){
		...
		$('#table1').click(function(){
			$('#action').val("sheet1");
			display();
			$('#headline').text("Оборотная ведомость по квартире");
			changeInputYearMonth("year");
		});
		$('#table2').click(function(){
			$('#action').val("sheet2");
			display();
			$('#headline').text("Должники по категориям");
			changeInputYearMonth("month");
		});
	...
	$('#apply').click(function(){
		...
		if (mass.length>1){
			$('#inputMonth').text(` ${monthNames[Number(mass[1])-1]} ${mass[0]} `);
		}
		else{
